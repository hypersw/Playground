...
export class Mouse extends Phaser.Physics.Arcade.Sprite {
  ...
  private createAnimations(): void {
    if (!this.scene.anims.exists('mouse-idle-down')) {
      const s = this.scene;
      // Sprite row order: S, W, N, E (rows 0-3)
      ...
      s.anims.create({ key: 'mouse-idle-right', frames: [{ key: 'mouse', frame: 10 }], frameRate: 1 });
      ...
      s.anims.create({ key: 'mouse-walk-right', frames: s.anims.generateFrameNumbers('mouse', { start: 9, end: 11 }), frameRate: 8, repeat: -1 });
    }
  }
